...
show that
\begin{multline}
E[e^{2}(n)] = r_{dd} - \mbf{W}^{T}(n)\mbf{r}_{xd}  
\\
- \mbf{r}^{T}_{xd}\mbf{W}(n) + \mbf{W}^{T}(n) R \mbf{W}(n)
\end{multline}
where
\begin{align}
r_{dd} &= E[d^2(n)]
\\
\mbf{r}_{xd} &= E[\mbf{X}(n)d(n)]
\\
...
By computing 
\begin{equation}
\frac{\partial E[e^{2}(n)]}{\partial \mbf{W}(n)}=0,
\end{equation}
show that the optimal solution for
%
\begin{equation}
\label{eq:wiener_opt}
\mbf{W}^*(n) = \min_{\mbf{W}(n)}E\sbrak{e^2(n)} = R^{-1} \mbf{r}_{xd}
\end{equation}
%
...
Let
\begin{equation}
\tilde {\mbf{W}}(n)= \mbf{W}(n) - \mbf{W}_{*}
\end{equation}
%
where $\mbf{W}_{*}$ is obtained in \eqref{eq:wiener_opt}. Also, according to the LMS algorithm,
\begin{align}
\label{eq:wn_update}
\mbf{W}(n+1)&=\mbf{W}(n)+ \mu \mbf{X}(n) e(n)
\\
e(n) &= d(n) - \mbf{X}^{T}(n)\mbf{W}(n)
\end{align}
%
\item
%
Show that
\begin{equation}
 E\sbrak{\tilde {\mbf{W}}(n+1)}=[I - \mu R]E\sbrak{\tilde {\mbf{W}}(n)}
\end{equation}
...
\lim_{n \to \infty}E\sbrak{\tilde {\mbf{W}}(n+1)} = 0 \iff \lim_{n\to \infty}[I - \mu \Lambda]^n = 0
\end{equation}
...
\tilde {\mbf{W}}(n)
 =
  \begin{bmatrix}
   \tilde{ \mbf{W}}_1(n) \\ \tilde {\mbf{W}}_2(n) \end{bmatrix}
\end{equation}
...
Show that 
\begin{equation}
E[\tilde {\mbf{W}}^{T}(n)\mbf{X}(n) \mbf{X}^{T}(n) \tilde {\mbf{W}}(n)] = E[\tilde {\mbf{W}}^{T}(n) R \tilde {\mbf{W}}(n)]
\end{equation}
%
...
=E[e_{*}^{2}(n)]
\\
 + E[\tilde {\mbf{W}}(n)\mbf{X}(n) \mbf{X}(n)^{T} \tilde {\mbf{W}}(n)^T] 
\\
- E[ \tilde {\mbf{W}}(n)\mbf{X}(n)e_{*}(n)] 
\\ 
- E[ e_{*}(n)\mbf{X}^{T}(n)\tilde {\mbf{W}}^T(n)]
\end{multline}
%
 where  
 \begin{align}
...
Show that
\begin{equation}
E\sbrak{ \tilde {\mbf{W}}(n)\mbf{X}(n)e_{*}(n)} = E\sbrak{ e_{*}(n)X^{T}(n)\tilde {\mbf{W}}^T(n)} = 0
\end{equation}
...
Show that
\begin{align}
E\sbrak{\tilde {\mbf{W}}^{T}(n) R \tilde {\mbf{W}}(n)} &= \text{trace}\brak{E\sbrak{\tilde {\mbf{W}}^{T}(n) R \tilde {\mbf{W}}(n)}}
\\
&= \text{trace}\brak{E\sbrak{\tilde {\mbf{W}}(n)\tilde {\mbf{W}}^{T}(n) }R}
\end{align}
...
\begin{equation}
\tilde {\mbf{W}}(n+1) = \sbrak{I - \mu \mbf{X}(n) X^{T}(n) }\tilde {\mbf{W}}(n) + \mu \mbf{X}(n)e_{*}(n)
\end{equation}
\item
Let $\mu^2 \to 0$.  Using \eqref{eq:eigen_decompose} and \eqref{eq:wiener_opt}, show that
\begin{multline}
E\sbrak{\tilde {\mbf{W}}(n+1)\tilde {\mbf{W}}^{T}(n+1)} 
\\
= \brak{I - 2\mu R}E\sbrak{\tilde {\mbf{W}}(n)\tilde {\mbf{W}}^{n}(n)}
\end{multline}
...
Show that 
\begin{equation}
\lim_{n\to\infty}E\sbrak{\tilde {\mbf{W}}(n)\tilde {\mbf{W}}^{T}(n)} = 0 \iff 0 < \mu < \dfrac{1}{\lambda_{max}}
\end{equation}
